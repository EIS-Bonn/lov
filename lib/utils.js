...
exports.relationSuggest  = function (object) {
    ...
    for (i = 0; i < object.length; i++){
      ...
      if(part['prefix'])html+='<a href=\'/dataset/bdo/vocabs/'+part['prefix']+'\'>('+part['prefix']+')</a>';
      html+='</div>';
    }
    return html;
}
...
//console.log(comment);
    ...
    for (i = 0; i < comment.length; i++){
      var part = comment[i];
      html+='<div>('+this.dateToYMD(part.createdAt)+')</span> <a href="/dataset/bdo/agents/'+part.agentId["name"]+'">'+part.agentId.name+'</a>: <span>'+part.body+'</div>';
    }
    return html;
}

//trillos
exports.displayDatasets  = function (datasets) {
//console.log(comment);

    var html='';
    for (i = 0; i < datasets.length; i++){
      var part = datasets[i];
      html+='<div>('+this.dateToYMD(part.createdAt)+')</span>: '+part.label+' <a href='+part.uri+'>'+part.uri+'</a></div>';
    }
    return html;
}
